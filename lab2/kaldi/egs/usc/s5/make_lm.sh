...
$KALDI_ROOT/tools/irstlm/scripts/build-lm.sh -i data/local/dict/lm_test.text -n 1 -o data/local/lm_tmp/lm_test_unigram.ilm.gz
$KALDI_ROOT/tools/irstlm/scripts/build-lm.sh -i data/local/dict/lm_test.text -n 2 -o data/local/lm_tmp/lm_test_bigram.ilm.gz

# Step 4.3 in folder data/local/nist_lm we store the comiled LM in APRA form
$KALDI_ROOT/tools/irstlm/src/compile-lm lm_train_unigram.ilm.gz --text=yes lm_train_unigram.lm
$KALDI_ROOT/tools/irstlm/src/compile-lm data/local/lm_tmp/lm_dev_unigram.ilm.gz -t=yes /dev/stdout | grep -v unk | gzip -c > data/local/nist_lm/lm_dev_unigram.arpa.gz
$KALDI_ROOT/tools/irstlm/src/compile-lm data/local/lm_tmp/lm_dev_bigram.ilm.gz -t=yes /dev/stdout | grep -v unk | gzip -c > data/local/nist_lm/lm_dev_bigram.arpa.gz
...
$KALDI_ROOT/tools/irstlm/src/compile-lm data/local/lm_tmp/lm_test_unigram.ilm.gz -t=yes /dev/stdout | grep -v unk | gzip -c > data/local/nist_lm/lm_test_unigram.arpa.gz
